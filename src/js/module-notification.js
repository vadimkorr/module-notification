 ...
 */
export let MNGroup = function(groupOptions) {
  var _defaultOptions = {
    name: "common",
    greedy: false
  };
 ...
 */
export let MNModule = function(moduleOptions) {
...
MNModule.prototype = {
  ...
  pushNotif: function(notifOptions) {
    ...
    function _onBeforeRemove(mnNotification) {
      var hasInd = _self.groups[mnNotification.options.group].hasNotif(
        mnNotification.id
      );
      if (hasInd != -1) {
        (function _decCount() {
 ...
 */
export let MNNotification = function(notifOptions) {
  var _self = this;
  function _getIcon() {
    var icons = {
      notice: "info-sign",
...
MNNotification.prototype = {
  ...
  appendToContainer: function(additionalOptions) {
    ...
    var _getDefaultTemplate = function(title, message, type, icon) {
      var template =
        "<div role='mn-alert' id='" +
        _self.id +
        "' class='mn-notification-container mn-alert mn-alert-" +
        type +
        " mn-alert-dismissible'>" +
